...
alias mine='fd --type file --extension mp4 --extension mkv --exec chmod --changes 0640'
alias top='btm --basic'
...
function tvdl
    if test -d ~/.config/tver-dl; and type -q tver-dl
        ...
        if not test -z "$result"
            printf "%s\n" $result | tee /dev/tty | yt-dlp --ignore-config --config-location ~/.config/tver-dl/yt-dlp.conf
        else
